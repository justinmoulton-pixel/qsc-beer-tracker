...
def check_login():
    ...
    # Create an empty placeholder to prevent the "flicker"
    ...
    if cookies is None:
        # Show a spinner in the placeholder while waiting for the browser
    ...
    # 3. UI Rendering (Only if not logged in)
    if not st.session_state.logged_in:
        # We fill the placeholder with the actual login UI
        with placeholder.container():
    ...
    if st.session_state.logged_in:
        placeholder.empty() # Final safety clear
        return st.session_state.user_info
    st.stop()
